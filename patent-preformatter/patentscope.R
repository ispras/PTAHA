# The script transforms Patentscope search result into common format (XLSX)
...
# Extracts year from the date; it should be DD.MM.YYYY formatted
GetYear <- function(date) {
  reduced <- substr(date, start = 1, stop = 10)
  year <- substr(reduced, start = 7, stop = 10)
...
# Patentscope data frame columns names
in_appl_id <- "Application Id"
in_appl_num <- "Application Number"
in_appl_date <- "Application Date"
in_publ_num <- "Publication Number"
in_publ_date <- "Publication Date"
in_country <- "Country"
in_title <- "Title"
in_ipc <- "I P C"
in_appl_names <- "Applicants"
in_inventors <- "Inventors"
in_prior_date <- "Priorities Data"

# Get input data
input_dframe <- readxl::read_excel(input_file, "ResultSet")

in_countries <- input_dframe[, in_country]
in_publ_nums <- input_dframe[, in_publ_num]
in_ipcs <- input_dframe[, in_ipc]
in_applicants <- input_dframe[, in_appl_names]
in_inventors <- input_dframe[, in_inventors]
...
full_ids <- paste(in_countries[[1]], in_publ_nums[[1]], sep = "-")
appl_years <- sapply(in_appl_dates, GetYear)

# Store values to output dataframe
colnames(out_dframe) <- out_col_names
out_dframe[, out_country] <- in_countries
out_dframe[, out_patent_num] <- in_publ_nums
...
out_file <- "patentscope-patents.xlsx"
